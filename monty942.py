...
prod_categories = df["prod_cat"].dropna().unique()
    ...
    [
        dcc.Tabs(
            ...
            children=[
                ...
                dcc.Tab(label="Klienci", value="tab-3"),
            ],
        ),
...
def render_tab2(df):
    fig = go.Figure(
        data=[go.Pie(labels=prod_grouped.index, values=prod_grouped.values)],
...
def render_tab3(df):
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(title="Sprzedaż wg płci"),
    )

    return html.Div(
        [
            html.H1("Klienci", style={"textAlign": "center"}),
